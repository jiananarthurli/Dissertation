...
\begin{figure}[h!]
	...
	\caption{The band structure of Graphene. The low energy approximation around $\mathbf{K}$ and $\mathbf{K'}$ has linear dispersion relation. The conduction and valence bands are gapless and the electron behave like Dirac fermions.}
...
$$ 
which depends solely on the the phase of the signal. All the noise at other frequencies are removed by the low-pass filter. In the experiments, the time-series data are modulated and demodulated with LIA written in LabVIEW programs.

\section{Data Analysis and Programming tools}

The pipeline for experimental data analysis, modeling and visualization is build from tools in various objected-oriented-programming (OOD) languages such as Python, Java and C++. Some other statistical tool and packages in MATLAB and R are also utilized for data analysis and visualization. 
...
\begin{figure}[h!]
	...
	\caption{Circuit diagram for electronic signal measurement.}
	\label{FIG:Circuit}
\end{figure}

% data aqcuisition and storage
% lockin
% details of each calculation
% packages used
% objected oriented programming languages such as python, Java and c++. 
% multicarrier density model using max-entropy models, and gradient descent for optimization method that are similar to machine learning models.
